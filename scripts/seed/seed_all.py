...
Run with:
    python manage.py shell -c "import importlib; importlib.import_module('scripts.seed.seed_all')"

This module exposes: seed_users(), seed_carriers_and_fleet(), seed_loads_and_payments(),
seed_education(), seed_comms(), seed_all()
"""
...
def seed_users():
    ...
    for i in range(3):
        get_or_create_user(f'driver{i+1}', f'driver{i+1}@example.com', 'driver', role='driver')
    return {'admin': admin, 'dispatcher': dispatcher}


def seed_carriers_and_fleet():
    ...
    for c in carriers:
        for j in range(2):
            Driver.objects.get_or_create(carrier=c, name=fake.name(), defaults={'license_number': str(100000+j), 'phone': fake.phone_number(), 'email': fake.email()})
            Truck.objects.get_or_create(carrier=c, plate_number=f'ABC{random.randint(100,999)}', defaults={'vin': str(uuid.uuid4()), 'model': fake.word().title(), 'year': random.randint(2008, 2024)})

    return {'carriers': carriers, 'brokers': brokers}


def seed_loads_and_payments(carriers_brokers):
    from apps.loads.models import Load
    from apps.payments.models import Payment

    carriers = carriers_brokers.get('carriers')
    brokers = carriers_brokers.get('brokers')
    loads = []
    for i in range(30):
    ...
    for l in loads:
        if random.random() < 0.7:
            Payment.objects.create(load=l, invoice_number=f'INV-{l.id}-{random.randint(1000,9999)}', amount=round(l.rate * random.uniform(0.9, 1.1), 2), status=random.choice(['pending', 'paid', 'rejected']))
...
def seed_education():
    ...
    for i in range(3):
        ExamTemplate.objects.get_or_create(name=f'Examen Demo {i+1}', defaults={'topic': random.choice(topics), 'num_questions': 20, 'difficulty_mix': {'easy': 40, 'medium': 40, 'hard': 20}})
...
def seed_comms(users_context=None):
    ...
    admin = users_context.get('admin') if users_context else None
    dispatcher = users_context.get('dispatcher') if users_context else None
    drivers = list(User.objects.filter(role='driver')[:10])
    for i in range(25):
        u = random.choice([admin, dispatcher] + drivers)
...
def seed_all():
    users = seed_users()
    carriers_brokers = seed_carriers_and_fleet()
    seed_loads_and_payments(carriers_brokers)
    seed_education()
    seed_comms(users)
